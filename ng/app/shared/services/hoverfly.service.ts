...
export class HoverflyService {
  ...
  setMode(modeSelection): void {
    this.http
      .put("/api/v2/hoverfly/mode", JSON.stringify({ mode: modeSelection }))
      .pipe(map((res) => res.json()))
      .subscribe(this.updateHoverfly(), httpErrorHandler(this.notifyService));
  }
  ...
  getMiddleware(): void {
    this.http
      ...
      .pipe(
        map((res) => res.json()),
        filter(
          (data: Middleware) => !!data.binary || !!data.script || !!data.remote
        ),
