...
describe("Service: Auth", () => {
  ...
  beforeEach(() => {
    ...
    backend.connections.subscribe(
      (connection) => (lastConnection = connection)
    );
  ...
  it("isAuthenticated should return true if response status is 200", () => {
    ...
    service
      .checkAuthenticated()
      .subscribe((authenticated) => (result = authenticated));
  ...
  it("should dispatch error notification action when login failed", () => {
    ...
    expect(notifyService.sendError).toHaveBeenCalledWith(
